...
sub import {
    ...
    if ( $first_module ) {
        my @module_args
          = ref $first_module->{ args } eq 'ARRAY' ? @{ $first_module->{ args } } : ();
...
sub _import_alias {
    ...
    for my $orig ( keys %{ $module->{ alias } } ) {
        ...
        my $orig_sub
          = $pkg . '::' . ( $suppress ? substr( $orig, 0, length( $orig ) - 1 ) : $orig );
    ...
    for my $module ( @_ ) {
        ...
        $ret{ $name }{ version } = $module->{ version }
          if ( defined $module->{ version }
            ...
            && $module->{ version } > $ret{ $name }{ version } )
          || !defined $ret{ $name }{ version };

        # optional
        $ret{ $name }{ optional } = $module->{ optional }
          if ( defined $module->{ optional }
            && defined $ret{ $name }{ optional }
            && $module->{ optional } > $ret{ $name }{ optional } )
          || !defined $ret{ $name }{ optional };
